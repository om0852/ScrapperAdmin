/**
 * Test Script for Jiomart Standardization API
 * usage: node test_standardization_jiomart.js
 * 
 * Pre-requisite: Jiomart Server must be running on port 4099
 */

import axios from 'axios';
import fs from 'fs';

const API_URL = 'http://localhost:4099/jiomartcategoryscrapper';
...
async function runTest() {
    console.log('🚀 Starting Jiomart API Standardization Test...');

    const payload = {
        pincode: '400703',
        urls: ['https://www.jiomart.com/c/groceries/biscuits-drinks-packaged-foods/chocolates-candies/29001'],
        store: true
    };
    ...
    try {
        ...
        if (response.status === 200 && response.data.status === 'success') {
            ...
            // Save Result to File
            fs.writeFileSync('api_test_result_jiomart.json', JSON.stringify(products, null, 2));
            console.log('\n💾 Test result saved to: api_test_result_jiomart.json');
        ...
        if (error.code === 'ECONNREFUSED') {
            console.error('❌ Could not connect to server. Is it running on port 4099?');
        } else {
