/**
 * Test Script for Instamart Standardization API
 * usage: node test_standardization_instamart.js
 * 
 * Pre-requisite: Instamart Server must be running on port 4400
 */

import axios from 'axios';
import fs from 'fs';

const API_URL = 'http://localhost:4400/instamartcategorywrapper';
...
async function runTest() {
    console.log('🚀 Starting Instamart API Standardization Test...');

    const payload = {
        pincode: '400703',
        url: 'https://www.swiggy.com/instamart/category-listing?categoryName=Fresh%20Vegetables&storeId=1404643&offset=0&filterName=&taxonomyType=Speciality%20taxonomy%201&showAgeConsent=false'
    };
    ...
    try {
        ...
        if (response.status === 200 && response.data.status === 'success') {
            ...
            // Save Result to File
            fs.writeFileSync('api_test_result_instamart.json', JSON.stringify(products, null, 2));
            console.log('\n💾 Test result saved to: api_test_result_instamart.json');
        ...
        if (error.code === 'ECONNREFUSED') {
            console.error('❌ Could not connect to server. Is it running on port 4400?');
        } else {
