...
function findCategoryByUrl(categoryUrl, urlMap) {
    ...
    // Strategy 3: Partial match on URL parameters
    if (categoryUrl.includes('categoryName')) {
        for (const [key, value] of urlMap.entries()) {
            if (key.includes('categoryName')) {
                // Both contain category parameter
                return value;
            }
        }
    }
