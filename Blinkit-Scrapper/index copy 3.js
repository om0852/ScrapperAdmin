                ...
                while (scrollAttempts < maxScrollAttempts) {
                    ...
                    if (currentCount === previousCount) {
                        ...
                        if (noChangeAttempts >= 4) {
                            log.info(`🛑 No new products found after ${noChangeAttempts} attempts, stopping.`);
                            break;
                        }
                    } else {
                        noChangeAttempts = 0;
                ...
                const products = await page.evaluate((globalDeliveryTime) => {
                    ...
                    productCards.forEach((card) => {
                        try {
                            ...
                            const rawWeight = weightEl ? weightEl.textContent.trim() : null;
                            let productWeight = rawWeight;
                            let quantity = 1;

                            if (rawWeight) {
                                const qtyMatch1 = rawWeight.match(/^(\d+)\s*[xX]\s*(.+)$/);
                                const qtyMatch2 = rawWeight.match(/^(.+)\s*[xX]\s*(\d+)$/);

                                if (qtyMatch1) {
                                    quantity = parseInt(qtyMatch1[1]);
                                    productWeight = qtyMatch1[2].trim();
                                } else if (qtyMatch2) {
                                    quantity = parseInt(qtyMatch2[2]);
                                    productWeight = qtyMatch2[1].trim();
                                }
                            }

                            // EXTRACT QUANTITY FROM ADD BUTTON (User Request)
                            const optionsEl = card.querySelector('div.tw-text-050.tw-font-050');
                            if (optionsEl) {
                                const optionsText = optionsEl.textContent.trim();
                                const qtyMatch = optionsText.match(/(\d+)\s*options?/i);
                                if (qtyMatch) {
                            ...
                            const productDeliveryTime = deliveryEl ? deliveryEl.textContent.trim() : null;
                            const deliveryTime = productDeliveryTime || globalDeliveryTime;
                            ...
                            const adEl = card.querySelector('img[src*="ad_without_bg.png"]');
                            const isAd = !!adEl;
