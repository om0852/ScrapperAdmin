...
async function scrapeCategory(context, category, config) {
    ...
    try {
        // Add random delay to prevent burst requests (1s to 4s)
    ...
    try {
        ...
        for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
            ...
            if (batchIndex < totalBatches - 1) {
                console.log('⏳ Waiting 1s before next batch to avoid rate limiting...');
                await delay(1000);
            }
        }
        ...
        const responsePayload = {
            status: 'success', // Changed from success: true to status: 'success' to match Blinkit
            pincode,
